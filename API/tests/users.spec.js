  ...
  it('should user loggin successfull', (done) => {
    chai
      ...
      .end((err, res) => {
        ...
        expect(res.body.data.info.firstname).to.equal('hbn');
        expect(res.body.data.info.lastname).to.equal('fiston');
        expect(res.body.data.info.email).to.equal('prosperfiston@gmail.com');
        done();
      });
  });
});

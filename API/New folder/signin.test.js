...
describe('POST /api/v1/auth/signup', () => {
    ...
    it('should return a 422 status if first name has more than 20 characters', done => {
        chai.request(app)
            .post('/api/v1/auth/signup')
            .send({
                first_name: 'somereallylongnamethatimadeupitistrulylong',
                last_name: 'Doe',
                email: 'desmondoe@gmail.com',
                password: 'pass',
                address: '17 Lagos'
    ...
    it('should return a 422 status if last name has more than 20 characters', done => {
        chai.request(app)
            ...
            .send({
                first_name: 'Desmond',
                last_name: 'somereallylongnamethatimadeupitistrulylong',
                email: 'desmondoe@gmail.com',
                password: 'pass',
                address: '17 Lagos'
            })
    ...
    it('should return a 422 status if email was not provided', done => {
        chai.request(app)
            ...
            .end((error, res) => {
                ...
                expect(res.body).to.have.keys('status', 'error');
                expect(res.body.status).to.deep.equal('error');
                expect(res.body.error).to.deep.equals('email was not provided');
                done();
            });
    });
    it('should return a 422 status if email is invalid', done => {
        chai.request(app)
            ...
            .end((error, res) => {
                ...
                expect(res.body).to.have.keys('status', 'error');
                expect(res.body.status).to.deep.equal('error');
                expect(res.body.error).to.deep.equals('invalid email');
                done();
            });
    });
    ...
    it('should return a 201 status if account was created', done => {
        chai.request(app)
            ...
            .end((error, res) => {
                ...
                expect(res.body.data.first_name).to.deep.equals('Desmond');
                expect(res.body.data.last_name).to.deep.equals('Doe');
                expect(res.body.data.email).to.deep.equals(
                    'desmonddoe@gmail.com'
                );
                done();
            });
    });
    ...
    it('Should return a 422 status if email was not provided', done => {
        chai.request(app)
            ...
            .end((error, res) => {
                ...
                expect(res.body.error).to.deep.equal('email was not provided');
                done();
            });
    });
    it('Should return a 422 status if password was not provided', done => {
    ...
    it('Should return a 422 status if email has invalid characters', done => {
        chai.request(app)
            ...
            .end((error, res) => {
                ...
                expect(res).to.have.status(422);
                expect(res.body.status).to.deep.equals('error');
                expect(res.body.error).to.deep.equal('invalid email');
                done();
            });
    });
...
describe('PATCH /api/v1/auth/reset-password/:token', () => {
    const wrongResetLink =
        '/api/v1/auth/reset-password/eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjYsInVzZXJFbWFpbCI6Im9zYWhvbm9ib2l0ZUBnbWFpbC5jb20iLCJpYXQiOjE1NTg3NTE5NzIsImV4cCI6MTU1ODgzNjU3Mn0.4dWAprRtYf_dFQ4EM1LiHyxn5qbSwSohoDsOWqS3d58';
